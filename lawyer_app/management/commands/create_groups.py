...
from lawyer_app.models import Lawyer, Client
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        groups_permissions = {
            ...
            'Client': ['view_case'],
            'Admin': ['add_lawyer', 'change_lawyer', 'delete_lawyer',
        ...
        for group_name, permissions in groups_permissions.items():
            ...
            for perm in permissions:
                ...
                content_type = ContentType.objects.get(model=model_name)  # Get model content type
                try:
                    permission = Permission.objects.get(codename=perm, content_type=content_type)
                    group.permissions.add(permission)
                except Permission.DoesNotExist:
                    self.stdout.write(self.style.WARNING(f'Permission "{perm}" does not exist for model "{model_name}"'))
