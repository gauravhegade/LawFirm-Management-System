...
from .models import Case, Client, Document, Lawyer
...
def search_clients(request):
    if (
        ...
        results = [
            {"id": client.client_id, "name": client.user.get_full_name()}
    ...
    if (
        ...
        if hasattr(request.user, "lawyer_profile"):
            cases = Case.objects.filter(
                lawyer=request.user.lawyer_profile, case_name__icontains=query
            )[:limit]

        elif hasattr(request.user, "client_profile"):
            cases = Case.objects.filter(
                client=request.user.client_profile, case_name__icontains=query
            )[:limit]
            ...
            if user is not None:
                if user.is_active:
                    ...
                    if profile_data and not profile_data.profile_complete:
                        ...
                        return redirect(
                            reverse(
                                f"lawyer_app:complete_{profile_data.__class__.__name__.lower()}_profile"
                            )
                        )
                    ...
                    form.add_error(
                        None, "Your account is not active. Please wait for approval."
                    )
            else:
...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            username = form.cleaned_data.get("username")
            messages.success(
                request, f"Account created for {username}! Please wait for approval."
            )
...
def complete_lawyer_profile(request):
    ...
    return render(
        request, "profile_completion/complete_lawyer_profile.html", {"form": form}
    )
...
def complete_client_profile(request):
    ...
    return render(
        request, "profile_completion/complete_client_profile.html", {"form": form}
    )
...
def lawyer_dashboard(request):
    ...
    clients = Client.objects.filter(
        cases__lawyer=lawyer, cases__status="approved"
    ).distinct()

    context = {"lawyer": lawyer, "clients": clients, "cases": cases, "user": True}
...
def client_profile(request, client_id):
    ...
    context = {"user": True, "client": client}
...
def client_dashboard(request):
    ...
    current_lawyers = Lawyer.objects.filter(
        cases__client=client, cases__is_active=True, cases__status="approved"
    ).distinct()

    past_lawyers = Lawyer.objects.filter(
        cases__client=client, cases__is_active=False
    ).distinct()
...
def lawyer_profile(request, lawyer_id):
    ...
    context = {"lawyer": lawyer, "user": False}
...
def approve_case(request):
    if request.method == "POST":
        ...
        try:
            case = Case.objects.get(
                case_id=case_id, client=request.user.client_profile, status="pending"
            )
    ...
    pending_cases = Case.objects.filter(
        client=request.user.client_profile, status="pending"
    )

    context = {"pending_cases": pending_cases, "user": False}
...
def upload_document(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if (
                (hasattr(request.user, "lawyer_profile"))
                and case.lawyer != request.user.lawyer_profile
            ) and (
                (hasattr(request.user, "client_profile"))
                and case.client != request.user.client_profile
            ):
                raise PermissionDenied(
                    "You do not have permission to upload documents for this case."
                )
...
def case_details(request, case_id):
    ...
    if case.status == "approved":
        ...
        context = {"case": None, "user": hasattr(request.user, "lawyer_profile")}
